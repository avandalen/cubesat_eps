...
enum pinMode_t {
	...
	biderectionalTrans = 	MAX_FUNCID_LVL_TRANS_BI,
...
enum tempSensorEnableMode_t {
	mode0 = MAX_TMPCTL_ENABLE_MODE_0,    // all disabled
	mode1 = MAX_TMPCTL_ENABLE_MODE_1,    // internal sensor enabled only
	mode2 = MAX_TMPCTL_ENABLE_MODE_2,    // external sensor 1 enabled only
	mode3 = MAX_TMPCTL_ENABLE_MODE_3,
	mode4 = MAX_TMPCTL_ENABLE_MODE_4,    // external sensor 2 enabled only
	mode5 = MAX_TMPCTL_ENABLE_MODE_5,
	mode6 = MAX_TMPCTL_ENABLE_MODE_6,
	mode7 = MAX_TMPCTL_ENABLE_MODE_7,    // all enabled
	modeNONE = 	0xffff
};
...
class MAX11300 {
	...
	public:
		 ...
		 * @param ADCrange - voltage range of MAX11300 pin
		 ...
		 */
		bool setPinModeMAX(uint8_t pin, pinMode_t mode);
		bool setPinModeMAX(uint8_t pin, pinMode_t mode, ADCRange_t ADCrange);
		bool setPinModeMAX(uint8_t pin, pinMode_t mode, ADCRange_t ADCrange, uint8_t differentialPin);
		...
		pinMode_t getPinModeMAX(uint8_t pin);
		...
		bool setTempSensorEnableMode(tempSensorEnableMode_t mode);
		tempSensorEnableMode_t getTempSensorEnableMode(void);

		bool setTempAveraging (tempSensor_t sensor, uint8_t samples);
		uint16_t getTempAveraging (tempSensor_t sensor);
		...
		bool burstAnalogDiffRead (uint8_t startPin, int16_t* samples, uint8_t size);
	...
	private:
		...
		uint16_t readRegisterSigned (uint8_t address, int16_t * values, uint8_t size);
